...
int main(){
	...
	infile.open("testbench/testcase_generator/BNE/test_data.txt");

	getline(infile, dummyLine);
	while(infile >> gld >> input_reg1 >> input_data1 >> input_reg2 >> input_data2 >>offset_address >> db >> filename ){
		outfile.open(("testbench/BNE_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/BNE_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		outfile << "bne $" << input_reg1 << " , $" <<input_reg2 <<" , branchoffset" << offset_address<< endl;
		...
		if (db == 'b'){  // testing backward branch 1 where backward branch is not taken
			...
			outfile << "bne $" << input_reg1 << " , $" <<input_reg1 <<" , branchoffset0000000C" <<endl;
			outfile <<"branchoffset00000020 : "<< endl <<"nop" <<endl;
			outfile <<"branchoffset00000024 : "<< endl << "li $2 , 0x57" << endl; 
			outfile << "jr   $0" << endl;
		}

		else if (db == 'c'){  // testing backward branch 1 where backward branch is taken
			outfile <<"nop" << endl;
			outfile <<"branchoffset00000018 : "<< endl << "li $2, 0x80000043" << endl;
			outfile << "bne $" << input_reg1 << " , $" <<input_reg2 <<" , branchoffset0000000C" <<endl;
			outfile <<"branchoffset00000020 : "<< endl <<"nop" <<endl;
			outfile <<"branchoffset00000024 : "<< endl << "li $2 , 0x57" << endl; 
			outfile << "jr   $0" << endl;
		}
