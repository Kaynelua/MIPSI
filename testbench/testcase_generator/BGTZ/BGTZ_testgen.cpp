...
int main(){
	...
	infile.open("testbench/testcase_generator/BGTZ/test_data.txt");

	getline(infile, dummyLine);
	while(infile >> gld >> input_reg >> input_data >> offset_address >> db >> filename ){
		outfile.open(("testbench/BGTZ_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/BGTZ_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		outfile << "bgtz $" << input_reg << " , branchoffset" << offset_address<< endl;
		...
		outfile <<"nop" << endl;
		outfile <<"branchoffset00000018 : "<< endl << "li $" << input_reg << ", 0x70000043" << endl;
		outfile <<"branchoffset0000001C : " << endl << "bgtz $" << input_reg << " , branchoffset0000000C" <<endl;
		outfile <<"branchoffset00000020 : "<< endl <<"nop" <<endl;
		outfile <<"branchoffset00000024 : "<< endl << "li $" << input_reg << ", 0x57" << endl; 
		outfile << "jr   $0" << endl;
		}

		else if (db == 'c' || db == 'o'){  // testing backward branch 2 where backward branch not taken
		outfile <<"nop" << endl;
		if(db=='c') {
			outfile <<"branchoffset00000018 : "<< endl << "li $" << input_reg << ", 0x80000048" << endl;
		}
		else if (db =='o'){
			outfile <<"branchoffset00000018 : "<< endl << "li $" << input_reg << ", 0x00000000" << endl;
		}
		outfile <<"branchoffset0000001C : " << endl << "bgtz $" << input_reg << " , branchoffset0000000C" <<endl;
		outfile <<"branchoffset00000020 : "<< endl <<"nop" <<endl;
		outfile <<"branchoffset00000024 : "<< endl << "li $" << input_reg << ", 0x57" << endl; 
