...
int main(){
	...
	infile.open("testbench/testcase_generator/BGEZ/test_data.txt");

	getline(infile, dummyLine);
	while(infile >> gld >> input_reg >> input_data >> offset_address >> db >> filename ){
		outfile.open(("testbench/BGEZ_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/BGEZ_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		outfile << "bgez $" << input_reg << " , branchoffset" << offset_address<< endl;
		...
		outfile <<"branchoffset0000001C : " << endl << "bgez $" << input_reg << " , branchoffset0000000C" <<endl;
