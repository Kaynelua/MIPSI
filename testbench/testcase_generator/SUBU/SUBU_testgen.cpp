...
int main(){
	...
	infile.open("testbench/testcase_generator/SUBU/test_data.txt");

	getline(infile, dummyLine);
	while(infile >> gld >> input_reg1 >> input_data1 >> input_reg2 >> input_data2 >> filename ){
		outfile.open(("testbench/SUBU_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/SUBU_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		outfile << "subu $2, $" 	<< input_reg1	 << ", $" <<input_reg2 	<<		  endl;
		
	
 		outfile << "jr   $0" 		<< endl;
/****************************************************************/
 		outfile_gld << "SUBU " << gld << " yhl15";
