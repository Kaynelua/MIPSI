...
int main(){
	...
	infile.open("testbench/testcase_generator/MFHI/test_data.txt");

	getline(infile, dummyLine);
	while(infile >> gld >> input_reg >> input_data >> dest_reg >> filename ){
		outfile.open(("testbench/MFHI_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/MFHI_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		outfile << "add $2,$0,$"<< dest_reg	<< 						  endl;
 		...
 		outfile << "jr   $0" 		<< endl;
/****************************************************************/
 		outfile_gld << "MFHI " << gld << " patrickjohncyh";
