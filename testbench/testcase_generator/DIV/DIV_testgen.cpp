...
int main(){
	...
	while(infile >> gld >> input_reg1 >> input_data1 >> input_reg2 >> input_data2 >> testHL>> filename ){
		outfile.open(("testbench/DIV_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/DIV_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		outfile << "div $" 	<< input_reg1	 << ", $" <<input_reg2 	<<		  endl;
