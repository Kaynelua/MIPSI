...
int main(){
	...
	infile.open("testbench/testcase_generator/SRL/test_data.txt");

	getline(infile, dummyLine);
	while(infile >> gld >> input_reg >> input_data >> shamt >> filename ){
		outfile.open(("testbench/SRL_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/SRL_tc/" + filename + ".mips.gld").c_str());
