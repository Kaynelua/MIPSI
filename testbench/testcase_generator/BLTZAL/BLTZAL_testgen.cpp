...
int main(){
	...
	infile.open("testbench/testcase_generator/BLTZAL/test_data.txt");

	getline(infile, dummyLine);
	while(infile >> gld >> input_reg >> input_data >> offset_address >> db >> filename ){
		outfile.open(("testbench/BLTZAL_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/BLTZAL_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		outfile <<"branchoffset0000001C : " << endl << "bltzal $" << input_reg << " , branchoffset0000000C" <<endl;
