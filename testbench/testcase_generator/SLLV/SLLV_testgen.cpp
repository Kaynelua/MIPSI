...
int main(){
	...
	infile.open("testbench/testcase_generator/SLLV/test_data.txt");



	getline(infile, dummyLine);
	while(infile >> gld >> input_reg >> input_data >> shift_reg >> shift_data >> filename ){
		outfile.open(("testbench/SLLV_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/SLLV_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		outfile << "sllv $2,$"		<< input_reg << ",$" << shift_reg << endl;
 		outfile << "jr   $0" << endl;

 	
/****************************************************************/
 		outfile_gld << "SLLV " << gld << " patrickjohncyh";
