...
int main(){
	...
	infile.open("testbench/testcase_generator/MTLO/test_data.txt");

	getline(infile, dummyLine);
	while(infile >> gld >> input_reg >> input_data >> dest_reg >> filename ){
		outfile.open(("testbench/MTLO_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/MTLO_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		outfile << "add $2,$0,$"<< dest_reg	<< 						  endl;
 		...
 		outfile << "jr   $0" 		<< endl;
/****************************************************************/
 		outfile_gld << "MTLO " << gld << " patrickjohncyh";
