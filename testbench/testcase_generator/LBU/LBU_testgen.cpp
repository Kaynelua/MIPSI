...
int main(){
	...
	infile.open("testbench/testcase_generator/LBU/test_data.txt");

	getline(infile, dummyLine);
	while(infile >> gld >> input_data >> base_reg >> base_data >> offset >> dest_reg >> test_type >> filename ){
		outfile.open(("testbench/LBU_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/LBU_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		if(test_type == "N"){	//testing basic functionality and special cases
			...
			outfile << "LBU   $2,0($1)" 			   << endl;	//load lsbyte of first instruction into R2
			...
			outfile << "LBU   $" << dest_reg << "," << offset    << "($" << base_reg << ")" << endl;	//load lsbyte of first instruction into R2
		}
 		...
 		outfile << "jr   $0" 		<< endl;
/****************************************************************/
 		outfile_gld << "LBU " << gld << " patrickjohncyh";
