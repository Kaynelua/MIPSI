...
int main(){
	...
	infile.open("testbench/testcase_generator/MULTU/test_data.txt");

	getline(infile, dummyLine);
	while(infile >> gld >> input_reg1 >> input_data1 >> input_reg2 >> input_data2 >> testHL>> filename ){
		outfile.open(("testbench/MULTU_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/MULTU_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		outfile << "multu $" 	<< input_reg1	 << ", $" <<input_reg2 	<<		  endl;
