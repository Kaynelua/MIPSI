...
int main(){
	...
	infile.open("testbench/testcase_generator/XORI/test_data.txt");

	getline(infile, dummyLine);
	while(infile >> gld >> input_reg1 >> input_data1 >> imm  >> type >> filename ){
		outfile.open(("testbench/XORI_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/XORI_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		if(type == "N"){		//basic functionality
 		...
 		outfile << "jr   $0" 		<< endl;
/****************************************************************/
 		outfile_gld << "XORI " << gld << " patrickjohncyh";
