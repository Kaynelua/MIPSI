...
int main(){
	...
	infile.open("testbench/testcase_generator/BLEZ/test_data.txt");

	getline(infile, dummyLine);
	while(infile >> gld >> input_reg >> input_data >> offset_address >> db >> filename ){
		outfile.open(("testbench/BLEZ_tc/" + filename + ".s").c_str());
		outfile_gld.open(("testbench/BLEZ_tc/" + filename + ".mips.gld").c_str());
...
/****************************************************************/
		...
		outfile << "blez $" << input_reg << " , branchoffset" << offset_address<< endl;
		...
		outfile <<"branchoffset0000001C : " << endl << "blez $" << input_reg << " , branchoffset0000000C" <<endl;
